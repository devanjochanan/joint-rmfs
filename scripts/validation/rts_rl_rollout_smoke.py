#!/usr/bin/env python3
"""Pure smoke for RTS rollout integration."""
...
def assert_current_cold_start_reward(row):
    reward_json = row["reward_json"]
    assert reward_json["reward_computed"] is False
    assert reward_json["reward_mode"] == "cold_start_paper_cycle_duration"
    assert reward_json["reward_horizon"] == "paper_cycle_duration"
    assert reward_json["reward_reference_required"] is False
    assert reward_json["cycle_reference_enabled"] is False
