...
def main():
    ...
    mask = build_action_mask(
        zone_ids,
        store_valid_by_zone={"A": True, "B": False},
        replenish_valid_by_zone={"A": False, "B": True},
    )
    assert len(mask) == 4
    ...
    assert any(row["replenish_store_action_valid"] > 0 for row in state.state_json["zone_rows"])
    ...
    assert any(row["store_action_valid"] > 0 for row in no_need_state.state_json["zone_rows"])
    assert all(row["replenish_store_action_valid"] == 0.0 for row in no_need_state.state_json["zone_rows"])
