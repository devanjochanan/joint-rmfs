...
def main():
    ...
    cold_start_events = [
        decision("cold_start", "rts_rl_explicit"),
        outcome("cold_start"),
    ]
    cold_start_events[1]["reward_json"] = {"reward_computed": False, "reward_value": None}
    cold_start_events[1]["paper_cycle_duration"] = 2.0
    cold_start_dataset = build_on_policy_training_steps(
        cold_start_events,
        required_policy_checkpoint_id="batch_000001",
    )
    assert cold_start_dataset.summary["trainable_step_count"] == 1
    assert cold_start_dataset.summary["rejected_reward_uncomputed_count"] == 0
    assert cold_start_dataset.steps[0].reward == -1.0
