...
class Intersection:
    ...
    def calculate_total_energy_per_direction(self):
        total_energy_horizontal = 0
        total_energy_vertical = 0

        for robot in self.horizontal_robots.values():
            total_energy_horizontal += robot.current_intersection_energy_consumption

        for robot in self.vertical_robots.values():
            total_energy_vertical += robot.current_intersection_energy_consumption

        return total_energy_horizontal, total_energy_vertical

    def calculate_average_energy_per_direction(self):
        total_energy_horizontal = sum(
            robot.current_intersection_energy_consumption for robot in self.horizontal_robots.values())
        total_energy_vertical = sum(
            robot.current_intersection_energy_consumption for robot in self.vertical_robots.values())

        average_energy_horizontal = total_energy_horizontal / len(
            self.horizontal_robots) if self.horizontal_robots else 0
        average_energy_vertical = total_energy_vertical / len(self.vertical_robots) if self.vertical_robots else 0

        return average_energy_horizontal, average_energy_vertical
