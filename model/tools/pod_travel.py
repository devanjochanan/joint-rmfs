...
def _connect(db_path):
    return connect(DEFAULT_DB_PATH if db_path == "warehouse.db" else db_path)
...
def initialize_pod_travel_table(timestamp: str, db_path="warehouse.db"):
    ...
    commit(conn)
    conn.close()
    debug_log("pod_travel table initialized.")
...
def clear_pod_travel(db_path="warehouse.db"):
    ...
    execute(cursor, f"DELETE FROM pod_travel_{TS}")

    commit(conn)
    conn.close()
    debug_log("All pod travel records have been cleared.")
...
def upsert_pod_travel(
    ...
    if existing:
        ...
        if updates:
            ...
            debug_log(f"Updated pod travel for pod {pod_id} (job {job_id}, robot {robot_id}, task {task}).")
        else:
        ...
        debug_log(f"Inserted new pod travel record for pod {pod_id} (job {job_id}, robot {robot_id}, task {task}).")
