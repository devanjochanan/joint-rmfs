...
def validate_rts_runtime_config(config: RTSRuntimeConfig) -> None:
    ...
    if config.policy_mode == "vrsla_teacher" and not config.rollout_enabled:
        raise ValueError("vrsla_teacher requires rollout_enabled=True")
    ...
    if config.policy_mode == "vrsla_teacher" and config.state_capture_mode == "minimal":
        raise ValueError("vrsla_teacher requires full RTS state capture")
    ...
    if config.policy_mode == "vrsla_teacher":
        if not config.committed_next_reservations_enabled:
            raise ValueError("vrsla_teacher requires committed_next_reservations_enabled=True")
        if config.zone_ids and "auto" in config.zone_ids and config.zone_ids != ("auto",):
            raise ValueError("rts zone_ids must be exactly ('auto',) or an explicit zone list")
    ...
    if config.policy_mode in {"rts_rl_explicit", "vrsla_teacher"} and config.zone_ids != ("auto",):
        validate_no_col_zone_ids(config.zone_ids, context=config.policy_mode)
