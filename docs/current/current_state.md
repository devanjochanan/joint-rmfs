...
- **Stage 3 Charging Bridge**: Added an explicit opt-in Salsa charging bridge for forced fixture-runtime validation. The bridge loads the canonical Salsa charging config, tracks charger availability and robot battery/status accounting, detects low-battery robots, assigns chargers deterministically, and reports validation metrics. It does not wire charging into default `setup()`/`tick()` behavior, does not move robots to chargers, and does not claim long-run charging performance.
