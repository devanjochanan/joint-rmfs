...
- The recovery phases (Phases 1-15) plus a targeted active job-queue regret-k scheduler patch have been implemented locally and verified using local dry runs and smoke tests. No long simulation, training campaign, benchmark, or output-equivalence run was performed.
...
- **Cold-Start Reward Normalization**: `cycle_reference.json` is optional legacy compatibility. The active v1 path records `reward_mode=cold_start_realized_cycle_time`, `reward_reference_required=false`, and `alpha_enabled=false`.
